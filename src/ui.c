...
font_o *font_create(uint8_t *data, int32_t bitmap_width, int32_t bitmap_height) {
	...
	// create texture
	font->texture = texture_create(bitmap, bitmap_width, bitmap_height, 1, (texture_params_t) {
									   TEXTURE_FILTER_LINEAR, TEXTURE_FILTER_LINEAR,
									   TEXTURE_WRAP_CLAMP_TO_EDGE, TEXTURE_WRAP_CLAMP_TO_EDGE
								   });
	
	return font;
}

font_o *font_load(string_t path) {
	uint8_t *data;
	...
	font_o *font = font_create(data, 512, 512);
...
void ui_init() {
	_style = (ui_style_t) {
		.font          = font_load("C:/Windows/Fonts/Arialbd.ttf"),
...
void ui_text(string_t text, vec2_t pos, float32_t scale, ui_anchor_e anchor) {
    ...
    render_state_set((render_state_t){ .depth_testing = false, .blendig = true, .face_culling = true, .wireframe = false });
