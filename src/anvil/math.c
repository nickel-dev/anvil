...
global int32_t _current_seed;

void randomf_seed(int32_t seed) {
    _current_seed = seed;
}

int32_t randomf() {
    return (_current_seed = (1664525 * _current_seed + 1013904223) % 4294967296);
}

int32_t randomf_range(int32_t low, int32_t high) {
    low = MIN(low, high);
    high = MAX(low, high);
	return (randomf() % (high - low + 1)) + low;
}
